...
hold on;
box on;
plot(x, br, 'k-s', 'LineWidth', lw, 'MarkerSize', ms);
plot(x, er, 'r-o', 'LineWidth', lw, 'MarkerSize', ms);
plot(x, ci, 'b-o', 'LineWidth', lw, 'MarkerSize', ms);
plot([-1 -1], [-10 2], '--', 'Color', [.5 .5 .5],'LineWidth', 1+lw)
plot([0 0], [-10 min(br)], '--', 'Color', [.5 .5 .5],'LineWidth', 1+lw)
plot([1 1], [-10 2], '--', 'Color', [.5 .5 .5],'LineWidth', 1+lw)
text(-.95, .19, '$$h_1$$','Interpreter','latex', 'fontsize', 35)
text(0.05, .19, '$$h^*$$','Interpreter','latex', 'fontsize', 35)
text(1.05, .19, '$$h_2$$','Interpreter','latex', 'fontsize', 35)
xlabel('$$h$$','Interpreter','latex', 'FontSize', 35)
ylim([0, 4])
set(gca,'XTickLabel','')
set(gca,'YTickLabel','')
set(gca, 'fontsize', 25)

legend('Bounded Risk', 'Empirical Risk', 'Confidence Interval', 'Location', 'north')
saveas(h, 'plots/risk-plot.fig')
saveas(h, 'plots/risk-plot.eps', 'eps2c')
%% result tables 
clc
clear
close all
